...
public class ThumbUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		Enumeration<String> files = mre.getFileNames(); // 전송한 파일들의 태그 명
		...
		// 게시글 정보 수정
		Thumbnail t = (Thumbnail) hmap.get("thumb"); // 다운 캐스팅 : Object -> Thumbnail
		...
		for (int i = changeNames.size() - 1; i >= 0; i--) { // 3, 2, 1, 0
			int j = changeNames.size() - 1 - i; // 0, 1, 2, 3

			if (changeNames.get(i) != null && j <= oldList.size() - 1) {
				// 원래 명단에 있는 사진이 변경되었을 경우
