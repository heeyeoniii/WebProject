...
public class ThumbSelectOne extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 게시글(BOARD) 정보 & 사진(ATTACHMENT) 정보를 JOIN해 가져오는 경우

		// 1. 두 객체를 모두 저장할 수 있는 통합형 VO를 만드는 방법
		// 한 번 객체를 만들어 두면 사용하기 편리하다.
		// 객체 자체의 크기가 커져 메모리 사용률이 올라 서비스가 느려질 수 있다.
		
		// 2. Map 객체를 활용하여 여러 클래스를 key와 value로 각각 묶어서 가져오는 방법
		// 여러 객체가 각각의 VO 형태를 유지하여 언제든지 필요한
		// 개별 VO들을 상황에 따라 묶어 사용할 수 있다.
		// key 로 객체를 간접 접근하기 때문에, 실제 객체의 내용이 변경되어도 이를 바로 확인하기 어렵다.
