...
<meta charset="UTF-8">
<title>브로드캐스트</title>
<script src="/api/resources/js/jquery-3.6.0.min.js"></script>
<style>
	#messageWindow {
...
<body>
	...
	function connection(){
		webSocket = new WebSocket('ws://1.212.197.133:3140/api/broadcast');
	...
	function send(){
		if ($inputMessage.val() == ""){
			...
			// "{ userId : " + $('#chat_id').val() +", msg : " + $inputMessage.val() + "}";
	...
	// 새로운 사용자가 접속할 경우
	// 사용자 리스트에 추가하기 위한 서버 비동기 통신
	function getUserList(){
		$.ajax({
			url : "/api/bcUserList.do",
	...
	// 사용자가 접속을 종료할 경우
	// 사용자 리스트에서 제거하기 위한 서버 비동기 통신
	function delUserList(){
		$.ajax({
			url: "/api/bcDelUser.do",
			data : {chat_id : $('#chat_id').val() },
			type : "post",
			success : function(data){
